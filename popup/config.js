...
function updateConfiguration() {
	...
	if (config.debug_mode) {
		log("Stored the following configuration: " + JSON.stringify(config));
	}

	// Send a message to the background process to apply the settings
	browser.runtime.sendMessage({ action: "reload"}).then(function (m) {
		if (config.debug_mode) log("Message from the background script: " +
			JSON.stringify(m));
	},
